...
using namespace std;
/*
    Dados los legajos y sueldos de los empleados de una empresa que como maximo tiene 100  empleados,
...
int main()
{
    ...
    */
    ...
    if (cantPos > 0)
    {
        ...
        mostrarTodos(sueldos, 100);

        // AG2
        int legBus;
        cout << "ingrese legajo a buscar ";
        cin >> legBus;
        if (legBus < 1 || legBus > 100)
            cout << "EL legajo no est� en el rango" << endl;
        else
        {
            if (sueldos[legBus - 1] != 0)
                cout << sueldos[legBus - 1] << endl;
            else
...
void mostrarTodos(int s[], int t)
{
    ...
    {
        if (s[i] != 0)
            cout << "Legajo: " << i + 1 << " Sueldo: " << s[i] << endl;
    }
}
